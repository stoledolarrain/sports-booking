...
module.exports = (app, db) => {
  ...
  app.post("/reservas/cancelar/:id", checkUser, async (req, res) => {
    try {
      const reserva = await db.reserva.findOne({ where: { id: req.params.id } });
      
      if (reserva) {
        // 1. Cambiamos el estado de la reserva a "cancelada"
        await db.reserva.update({ estado: "cancelada" }, { where: { id: reserva.id } });
        
        // 2. ¡Volvemos a liberar el horario para que otro lo use!
        await db.horario.update({ disponible: true }, { where: { id: reserva.horario_id } });
      }
